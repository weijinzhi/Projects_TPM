...
<script type="text/javascript">
					...
					if (arr.length <= 0) {
						...
						$.messager.confirm('提示信息', '确认删除?', function(r) {
							if (r) {
								...
								$.post('json/Nation-delete', {
									ids : ids
								}, function(result) {
									//1 刷新数据表格 
									$('#nations').datagrid('reload');
									//2 清空idField   
									$('#nations').datagrid('unselectAll');
									//3 给提示信息 
									$.messager.show({
										title : '提示信息!',
										msg : '操作成功!'
	...
	function doSearch() {
		$('#nations').datagrid('load', {
			nationName: $('#nationName').val(),
		});
	}
	
	//点击清空按钮出发事件
	function clearSearch() {
		$("#nations").datagrid("load", {});//重新加载数据，无填写数据，向后台传递值则为空
...
<body>
	...
	<div id="lay" class="easyui-layout"
		...
		<div region="north" title="民族查询" collapsed=true style="height: 60px;">
			<form id="search" action="" method="post">
			
				部门:&nbsp;&nbsp;&nbsp;<input name="nationName" class="easyui-validatebox"
					id="nationName" value="" /> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
